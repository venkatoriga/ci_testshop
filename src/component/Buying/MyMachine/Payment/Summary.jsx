...
console.log('productDETAILS======>',productDETAILS);
  ...
  const paytoken = async () => {
    try {
      ...
      let url = `https://devextension.origa.market/fullpaymentredirect?id=${productDETAILS?.machine_details?.data?.product?.id}&buyMachineId=${buyMachineId}`
      const inputpaytoken = {
        "amount": productDETAILS?.price_details?.machine_total_price,
        "createdby": name,
        "mobileno": number,
        "paymentmethod": "PAGE",
        "paymentstatus": "PEN",
  ...
  const updateTokenAmount = (paymentRes) => {
    try {
      ...
      const inputbuymachine = {
        ...
        "fullpaymentamount": (productDETAILS?.price_details?.machine_total_price),
          ...
          <div className="summary-wrap">
            ...
            <div className="content-wrap pt-5">
              ...
              <div className="bottom-wrap">
                ...
                <div className="bottom-wrap">
                <div className="top-heading">
                  <div className="heading-600-24 heading-600-24-20 heading-600-24-16 heading-600-24-14 select-heading">Confirm Order Summary</div>
                  <div className="edit-address heading-600-14 heading-600-14-12"><span className='editicon curser-pointer' onClick={() => window.location = `/buy/new-address?id=${productId}&buyMachineId=${buyMachineId}`}>{editIcon({ width: 24, height: 24 })}</span> change Address</div>
                </div>
                    ...
                    <div className='detail-lists' key={index}>
                      ...
                      <div className="check-details">
                        ...
                        <div className="contact-details">
                           <div className="heading-400-14-12 phone"><span>{phoneIcon({ width: 24, height: 24 })}</span>{productDETAILS?.delivery_address?.phone && <span className="heading-400-14-12 op-60">&nbsp;{productDETAILS?.delivery_address?.phone}</span>}</div>
                           {/* <div className="heading-400-14-12 emailby"><span>{emailIcon({ width: 24, height: 24 })}</span> {productDETAILS?.delivery_address?.email && <span className="heading-400-14-12 op-60">&nbsp;{productDETAILS?.delivery_address?.email}</span>}</div> */}
                        </div>
                      </div>
              ...
              <div className="product-wrap">
                <div className="left-wrap">
                  ...
                  <div className="detail-wrap">
                    ...
                    <div className="price-wrap">
                      ...
                      <div className="price-item">
                        <div className="heading-600-14 price">₹{priceConvert(productDETAILS?.price_details?.machine_price)}</div>
                        {productDETAILS?.price_details?.token_payment_status !== "PAYMENT_SUCCESS" ? (
                          <div className="heading-600-14 price">₹{priceConvert(productDETAILS?.price_details?.full_amount)}</div>
                        ) : (
                          <div className="heading-600-14 price">₹{priceConvert(productDETAILS?.price_details?.balance_amount)}</div>
                        )}
                ...
                <div className="right-wrap">
                  <div className="heading-600-20 amount-heading">  Amount Summary</div>
                  <div className="price-item">
                    <div className="heading-400-16-12 title">Balance Price</div>
                    {productDETAILS?.price_details?.token_payment_status !== "PAYMENT_SUCCESS" ? (

                      <div className="heading-400-16-12 price">₹{priceConvert(productDETAILS?.price_details?.full_amount)}</div>
                    ) : (
                      <div className="heading-400-16-12 price">₹{priceConvert(productDETAILS?.price_details?.balance_amount)}</div>
                    )}
                  ...
                  <div className="price-item">
                    ...
                    {productDETAILS?.price_details?.token_payment_status !== "PAYMENT_SUCCESS" ? (
                      ...
                      <div className="heading-400-16-12 price">₹{priceConvert(productDETAILS?.price_details?.full_gst_amount)}</div>
                    ) : (
                      <div className="heading-400-16-12 price">₹{priceConvert(productDETAILS?.price_details?.balance_gst_amount)}</div>
                    )}

                  </div>
                  <div className="price-item">
                    <span className="heading-400-16-12 title">Delivery Charges</span>
                    {productDETAILS?.price_details?.delivery_charges_excluded ? (<strike className="heading-400-16-12 price">₹ {productDETAILS?.price_details?.delivery_charges ? priceConvert(productDETAILS?.price_details?.delivery_charges) : '1,00,000'}</strike>
                    ) : (<span className="heading-400-16-12 price">₹ {productDETAILS?.price_details?.delivery_charges ? priceConvert(productDETAILS?.price_details?.delivery_charges) : '1,00,000'}</span>
                    )}
                  </div>
                  {/* <div className="price-item">
                    <span className="heading-400-16-12 title">Repairing cost</span>
                    {productDETAILS?.price_details?.repairing_cost_excluded ? (<strike className="heading-400-16-12 price">₹ {productDETAILS?.price_details?.repairing_cost ? priceConvert(productDETAILS?.price_details?.repairing_cost) : '1,00,000'}</strike>
                    ) : (<span className="heading-400-16-12 price">₹ {productDETAILS?.price_details?.repairing_cost ? priceConvert(productDETAILS?.price_details?.repairing_cost) : '1,00,000'}</span>
                    )}
                  </div>
                  <div className="price-item">
                    <span className="heading-400-16-12 title">1 Year AMC</span>
                    {productDETAILS?.price_details?.amc_cost_excluded ? (<strike className="heading-400-16-12 price">₹ {productDETAILS?.price_details?.amc_cost ? priceConvert(productDETAILS?.price_details?.amc_cost) : '1,00,000'}</strike>
                    ) : (<span className="heading-400-16-12 price">₹ {productDETAILS?.price_details?.amc_cost ? priceConvert(productDETAILS?.price_details?.amc_cost) : '1,00,000'}</span>
                    )}
                  </div> */}
                  <div className="total-wrap">
                    <div className="heading-600-16 title">Total</div>
                    <div className="heading-600-16 total-price">₹{priceConvert(productDETAILS?.price_details?.machine_total_price)}</div>
                  </div>
                </div>
              </div>
